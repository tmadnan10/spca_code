...
public class CustomSparkPCA implements Serializable {
     ...
     public static void main(String[] args) {
		 ...
		 */
	     ...
	     SparkConf conf = new SparkConf().setAppName("CustomSparkPCA").setMaster("local[*]");
...
//           });
		   ...
		   nCentralC = centralYtX.times(invXtX_central);
          
           for (int p = 0; p < nCols; p++) {
               for (int q = 0; q < nPCs; q++) {
                   maxWnew = Math.max(Math.abs(nCentralC.getQuick(p, q)), maxWnew);
               }
           }
           for (int p = 0; p < nCols; p++) {
               for (int q = 0; q < nPCs; q++) {
                   dw = Math.max(Math.abs(centralC.getQuick(p, q) - nCentralC.getQuick(p, q)), dw);
               }
           }
           ...
           dw /= (sqrtEps + maxWnew);
           System.out.println("dw in round "+round+": "+dw);
           if (dw <= tolerance) { //convergence achieved
               System.out.println("Convergence Achieved in "+(round+1) +" rounds");

               long endTime = System.currentTimeMillis();
               double iterTime = (endTime-startTime)/1000.0;
               System.out.println("Iteration time in "+(round+1) +" :"+iterTime);
           }
           long endTime = System.currentTimeMillis();
           double iterTime = (endTime-startTime)/1000.0;
           System.out.println("Iteration time in "+(round+1) +" :"+iterTime);
